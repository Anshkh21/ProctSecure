...
const AdminDashboard = () => {
  ...
  const handleAdminReset = async (collection, label) => {
    ...
    setIsResetting(true);
    try {
      const { data } = await apiClient.delete(`/admin/reset/${collection}`);
  ...
  const fetchProctors = async () => {
    try {
      const { data } = await apiClient.get('/admin/proctors');
      setProctors(data);
    } catch (error) {
  ...
  const handleInviteProctor = async (e) => {
    ...
    try {
      const { data } = await apiClient.post('/admin/proctors/invite', inviteForm);
      ...
      toast({
        title: 'Error',
        description: error.response?.data?.detail || 'Network error occurred',
        variant: 'destructive'
      });
    } finally {
      setSubmitting(false);
  ...
  const handleDeleteProctor = async (proctorId, proctorName) => {
    ...
    try {
      await apiClient.delete(`/admin/proctors/${proctorId}`);
      toast({
        title: 'Success',
      ...
      toast({
        title: 'Error',
        description: error.response?.data?.detail || 'Failed to remove proctor',
        variant: 'destructive'
      });
    }
  };
              ...
              <CardContent>
                ...
                <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
                  ...
                  onClick={async () => {
                      ...
                      setIsResetting(true);
                      try {
                        const { data } = await apiClient.delete(`/admin/reset/proctor/${selectedProctorForReset}`);
