...
const ExamInterface = () => {
  ...
  const verificationKey = `examVerification:${examId}`;
  ...
  const studentFlagsRef = useRef([]);
  const submitExamRef = useRef(null); // Ref to avoid stale closure in timer
  ...
  const savePendingAnswers = useCallback(async () => {
    ...
    const pendingAnswers = Object.entries(answersRef.current).filter(([questionId, answer]) => {
      return lastSavedAnswersRef.current[questionId] !== answer;
    });
    ...
    pendingAnswers.forEach(([questionId, answer]) => {
      lastSavedAnswersRef.current[questionId] = answer;
    });
  }, []);
  ...
  useEffect(() => {
    studentFlagsRef.current = studentFlags;
  }, [studentFlags]);
  ...
  useEffect(() => {
    ...
    fetchStudentFlags();
    const interval = setInterval(fetchStudentFlags, 30000);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      savePendingAnswers().catch((error) => {
  ...
  // Re-attach streams to video elements after exam UI has mounted
  useEffect(() => {
    if (!hasStarted) return;
    if (videoRef.current && webcamStreamRef.current) {
      videoRef.current.srcObject = webcamStreamRef.current;
