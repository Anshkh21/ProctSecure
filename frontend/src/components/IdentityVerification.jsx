...
const IdentityVerification = () => {
  ...
  const [roomScanPassed, setRoomScanPassed] = useState(false);
  ...
  const captureCurrentFrame = () => {
    ...
    if (video.videoWidth === 0 || video.videoHeight === 0) {
      toast({
        title: "Camera Error",
        description: "Video stream is not ready yet. Please try again.",
        variant: "destructive"
      });
      return null;
    }
  ...
  const handleRoomScan = async () => {
    ...
    setIsProcessing(true);
    try {
      const response = await axios.post(`${API}/verify/room-scan`, {
        image_data: imageData
      }, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
      ...
      setRoomScanPassed(false);
      toast({
        title: "Room Scan Error",
        description: error.response?.data?.detail || "Failed to verify room scan. Please try again.",
        variant: "destructive"
      });
    } finally {
      setIsProcessing(false);
  ...
  const handleCompleteVerification = async () => {
    if (!examId || !idDocument || !capturedFaceImage || !roomScanImage) {
      toast({
        title: "Verification Incomplete",
        description: "Please finish ID, face, and room scan checks before continuing.",
        variant: "destructive"
      });
      return;
    }
    ...
    if (!consentAccepted) {
      toast({
        ...
        description: "Please accept the proctoring consent to continue.",
        variant: "destructive"
      });
      return;
    }
    ...
    setIsProcessing(true);
    try {
      const response = await axios.post(`${API}/verify/complete`, {
        exam_id: examId,
        id_image_data: idDocument,
        face_image_data: capturedFaceImage,
        room_image_data: roomScanImage,
      ...
      localStorage.setItem(`examVerification:${examId}`, JSON.stringify({
      ...
      toast({
        ...
        description: error.response?.data?.detail || "Failed to complete verification.",
        variant: "destructive"
      });
    } finally {
      setIsProcessing(false);
  ...
  const canProceed = () => {
    switch (currentStep) {
      ...
      case 4: return roomScanPassed;
      case 5: return roomScanPassed && consentAccepted;
            ...
            <div className="bg-white rounded-lg p-6 text-center space-y-4">
              ...
              <div className="flex justify-center space-x-4">
                ...
                >
                   ...
                   {isProcessing ? 'Analyzing Room...' : 'Capture Room Scan'}
                </Button>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-4 gap-4">
                ...
                <div className="text-center p-4 bg-green-50 rounded-lg">
                  ...
                  <p className="text-sm text-green-700">{roomScanPassed ? 'Scanned' : 'Pending'}</p>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-lg p-6 text-center">
                ...
                <p className="text-blue-100 mb-4">
                  Your verification data will be checked one final time before the exam session begins.
                </p>
                <label className="flex items-start gap-3 rounded-lg bg-white/10 p-4 text-left mb-4">
                  <input
                    type="checkbox"
                  ...
                  <span className="text-sm text-blue-50">
                    I consent to webcam, microphone, screen, and proctoring data being used for exam monitoring.
                  </span>
                </label>
                <Button 
                   variant="secondary" 
                   ...
                   disabled={isProcessing || !consentAccepted || !roomScanPassed}
                   className="font-bold"
                >
                   {isProcessing ? 'Finalizing Verification...' : 'Start Exam Now'} <ArrowRight className="ml-2 w-4 h-4" />
                </Button>
              </div>
            </div>
          </div>
