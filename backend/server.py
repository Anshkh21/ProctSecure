...
FRAME_STORAGE_DIR = ROOT_DIR / "runtime_frames"
FRAME_STORAGE_DIR.mkdir(exist_ok=True)
...
# JWT Configuration â€” fail fast if secret is not set
_JWT_SECRET_RAW = os.environ.get('JWT_SECRET')
if not _JWT_SECRET_RAW:
    raise RuntimeError(
        "FATAL: JWT_SECRET environment variable is not set. "
        "Add a strong random secret to backend/.env before starting the server."
    )
...
def decode_base64_image_to_bgr(image_data: str) -> np.ndarray:
    if "," in image_data:
...
def get_frame_storage_path(session_id: str, frame_type: str) -> Path:
    return FRAME_STORAGE_DIR / f"{session_id}_{frame_type}.txt"

def write_latest_frame(session_id: str, frame_type: str, image_data: Optional[str]):
    frame_path = get_frame_storage_path(session_id, frame_type)
    if not image_data:
        if frame_path.exists():
            frame_path.unlink()
        return

    frame_path.write_text(image_data, encoding="utf-8")

def read_latest_frame(session_id: str, frame_type: str) -> Optional[str]:
    frame_path = get_frame_storage_path(session_id, frame_type)
    if not frame_path.exists():
        return None
    return frame_path.read_text(encoding="utf-8")

def clear_session_frame_cache(session_id: str):
    for frame_type in ("webcam", "screen"):
        frame_path = get_frame_storage_path(session_id, frame_type)
        if frame_path.exists():
            frame_path.unlink()

async def analyze_room_scan(room_image_data: str) -> Dict[str, Any]:
    room_image = decode_base64_image_to_bgr(room_image_data)
    analysis = await run_in_threadpool(proctor_model.enhanced_analyze_frame, room_image)
    ...
    object_violations = analysis.get("object_detection", {}).get("violations", [])
    ...
    message = (
        ...
        if is_valid
        else "Room scan failed. Please remove flagged issues and try again."
    )
...
# NOTE: ExamCreateRequest is defined later (after QuestionItem) with the 'questions' field.
# The incomplete duplicate that used to live here has been removed.
...
async def delete_exam(exam_id: str, current_user: dict = Depends(require_proctor)):
    """Delete an exam — proctor must own the exam."""
    exam = await db.exams.find_one({"id": exam_id})
    if not exam:
        raise HTTPException(status_code=404, detail="Exam not found")
    if exam.get("proctor_id") != current_user["id"]:
        raise HTTPException(status_code=403, detail="You can only delete your own exams")
    
    await db.exams.delete_one({"id": exam_id})
    # Also delete associated questions and enrollments
    await db.questions.delete_many({"exam_id": exam_id})
    await db.exam_enrollments.delete_many({"exam_id": exam_id})
...
async def get_exam_details(exam_id: str, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        logger.error(f"get_exam_details error for exam {exam_id}: {e}")
...
async def get_proctor_students(proctor: dict = Depends(require_proctor)):
    ...
    exam_map = {exam["id"]: exam for exam in proctor_exams}
    ...
    student_ids = list({session["student_id"] for session in sessions})
    students = await db.users.find({"id": {"$in": student_ids}}).to_list(1000)
    student_map = {student["id"]: student for student in students}

    session_ids = [session["id"] for session in sessions]
    flag_counts_raw = await db.proctoring_flags.aggregate([
        {"$match": {"session_id": {"$in": session_ids}}},
        {"$group": {"_id": "$session_id", "count": {"$sum": 1}}}
    ]).to_list(1000)
    flag_count_map = {entry["_id"]: entry["count"] for entry in flag_counts_raw}
    ...
    for session in sessions:
        ...
        if student:
            ...
            # Recalculate score only when stored values are missing.
            score = session.get("score", 0)
            total_points = session.get("total_points", 0)
            percentage = session.get("percentage", 0)

            if session.get("status") == "completed" and (
                session.get("score") is None or
                session.get("total_points") in (None, 0) or
                session.get("percentage") is None
            ):
            ...
            result.append({
                ...
                "flag_count": flag_count_map.get(session["id"], 0),
...
async def start_exam_session(
    ...
    if current_user["role"] == "student":
        ...
        verification_record = await db.exam_verifications.find_one({
            "student_id": current_user["id"],
            "exam_id": exam_id,
            "status": "verified",
            "consumed_at": None
        ...
        if not verification_record:
            raise HTTPException(
                status_code=403,
                detail="Verification required before starting the exam."
            )
    ...
    await db.exam_sessions.insert_one(session.model_dump())
    clear_session_frame_cache(session.id)

    if current_user["role"] == "student":
        await db.exam_verifications.update_one(
...
@api_router.post("/verify/room-scan", response_model=VerificationResponse)
async def verify_room_scan(
    request: IdVerificationRequest,
    current_user: dict = Depends(require_student)
):
    try:
        room_scan_result = await analyze_room_scan(request.image_data)
        return VerificationResponse(
            is_valid=room_scan_result["is_valid"],
            message=room_scan_result["message"],
            warnings=room_scan_result["warnings"],
        )
    except Exception as e:
        print(f"Error in room scan verification: {e}")
        return VerificationResponse(
            is_valid=False,
            message="Room scan could not be completed. Please try again.",
            warnings=[str(e)],
        )

@api_router.post("/verify/complete", response_model=VerificationResponse)
async def complete_verification(
    ...
    try:
        if not request.consent_accepted:
            return VerificationResponse(
                is_valid=False,
                message="You must accept the proctoring consent before starting the exam.",
                warnings=["Consent not accepted"],
            )
        ...
        id_image = decode_base64_image_to_bgr(request.id_image_data)
        face_image = decode_base64_image_to_bgr(request.face_image_data)

        id_name_result = await run_in_threadpool(
            proctor_model.verify_id_name,
            id_image,
            current_user["name"]
        )
        if not id_name_result.get("match"):
            return VerificationResponse(
                is_valid=False,
                message="ID verification failed. Name on the ID card did not match your account.",
                warnings=["Name mismatch on ID card"],
            )

        face_analysis = await run_in_threadpool(proctor_model.analyze_frame, face_image)
        if face_analysis.get("face_count") != 1:
            return VerificationResponse(
                is_valid=False,
                message="Face verification failed. Please ensure exactly one face is visible.",
        ...
        return VerificationResponse(
            is_valid=True,
            message="Verification completed successfully. You can now start the exam.",
            warnings=[],
        )
...
# ---------------------------------------------------------------------------
# Shared helper: classify a proctoring warning into (flag_type, severity)
# ---------------------------------------------------------------------------
def _classify_warning(warning: str) -> tuple:
    """Map a warning description to (flag_type, severity)."""
    ...
    if "Dark" in warning or "Low light" in warning:
        return "environment_issue", "low"
    if "blurry" in warning:
        return "video_quality", "low"
    if "Mouth open" in warning or "Talking" in warning:
...
async def monitor_session(
    ...
    try:
        ...
        logger.error(f"Monitor session error for {session_id}: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def analyze_frame_enhanced(
    ...
    try:
        ...
        if current_user.get("role") == "student" and session.get("student_id") != current_user.get("id"):
        ...
        write_latest_frame(session_id, "webcam", request.image_data)
        write_latest_frame(session_id, "screen", request.screen_data if request.screen_data else None)
        ...
        await db.exam_sessions.update_one({'id': session_id}, update_operation)
            
        # Handle Standard Flags using shared helper
        if all_warnings:
            for warning in all_warnings:
                flag_type, severity = _classify_warning(warning)
                evidence = request.image_data if flag_type != "audio_violation" else None
...
async def submit_exam(
    ...
    try:
        ...
        clear_session_frame_cache(session_id)
...
async def get_proctor_analytics(current_user: dict = Depends(require_proctor)):
    proctor_id = current_user["id"]

    # Scope sessions to this proctor only (was incorrectly fetching all sessions)
    sessions = await db.exam_sessions.find({"proctor_id": proctor_id}).to_list(10000)
    
    # Scope questions to this proctor's exams only
    proctor_exams = await db.exams.find({"proctor_id": proctor_id}).to_list(1000)
    proctor_exam_ids = [e["id"] for e in proctor_exams]
    all_questions = await db.questions.find({"exam_id": {"$in": proctor_exam_ids}}).to_list(10000)
...
async def get_session_live_feed(
    ...
    """
    session = await db.exam_sessions.find_one({"id": session_id, "proctor_id": current_user["id"]})
    if not session:
        raise HTTPException(status_code=404, detail="Session not found")
        
    return {
        "webcam_frame": read_latest_frame(session_id, "webcam"),
        "screen_frame": read_latest_frame(session_id, "screen"),
...
async def reset_proctor_data(
    ...
    """
    try:
        # Fetch session IDs FIRST (before any deletions) to avoid read-after-delete bug
        proctor_sessions = await db.exam_sessions.find(
            {"proctor_id": proctor_id}, {"id": 1}
        ).to_list(10000)
        proctor_session_ids = [s["id"] for s in proctor_sessions]

        # Now delete in dependency order: flags → sessions → exams
    ...
    try:
        if collection_name == "exams":
            ...
            if is_admin:
                ...
                # Flags are not directly tied to proctor_id â€” find session IDs owned by this proctor first
...
async def ensure_indexes():
    index_specs = [
        (db.exam_sessions, "id", {"unique": True}),
        (db.exam_sessions, [("student_id", 1), ("exam_id", 1), ("status", 1)], {}),
        (db.exam_sessions, "exam_id", {}),
        (db.exam_sessions, "proctor_id", {}),
        (db.exam_sessions, "last_active", {}),
        (db.proctoring_flags, "session_id", {}),
        (db.proctoring_flags, [("student_id", 1), ("timestamp", -1)], {}),
        (db.exam_enrollments, [("exam_id", 1), ("student_id", 1)], {}),
        (db.exam_verifications, [("student_id", 1), ("exam_id", 1)], {"unique": True}),
        (db.users, "email", {}),
    ]

    for collection, spec, options in index_specs:
        try:
            await collection.create_index(spec, **options)
        except Exception as error:
            logger.warning(f"Index creation warning for {collection.name} ({spec}): {error}")


# ---------------------------------------------------------------------------
# Global exception handler — returns clean JSON instead of HTML traceback
# ---------------------------------------------------------------------------
